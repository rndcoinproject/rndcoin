...
#if defined(NDEBUG)
# error "Rndcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7a77361142e62ad48c91d7efe0c711d9be612533063b122b8441aec554e8b21f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rndcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rndcoin Signed Message:\n";
...
{
    // Rndcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Rndcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Rndcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Rndcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rndcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rndcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7c9f66bbec25dd89e13d16ec3c91d05af2013d2b37283f6acca0a8e82100834b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0469f2ada24d42c8b20bfad153656bf6efe49fd8ccee389e7de1e34d6866245dc5ec7f19aac73bcf022ee5408c35065e1bfaff0310451d9bcf65f0bc1291353f05") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1572329122;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084534001;

        if (fTestNet)
        {
            block.nTime    = 1572329065;
            block.nNonce   = 386134736;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x53420508960a82d9b0f33540c6d8d666ccdb7297c49957ea8e75a63641ebd031"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xcb, 0xb1, 0xda }; // Rndcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RndcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RndcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RndcoinMiner(CWallet *pwallet)
{
    printf("RndcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rndcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RndcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RndcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RndcoinMiner, pwallet));
}
